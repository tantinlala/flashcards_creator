...
def main():
    ...
    parser.add_argument('files', nargs='+', type=str, help='The files or glob patterns to convert to flashcards (e.g. ./*.png, ./screenshot*)')
    ...
    # Expand glob patterns into actual file paths (preserve order, remove duplicates)
    expanded_files = []
    seen = set()
    for pattern in args.files:
        matches = glob.glob(pattern, recursive=True)
        if not matches:
            # If no glob matches but the literal path exists, keep it
            if os.path.exists(pattern):
                matches = [pattern]
            else:
                print(f"Warning: pattern or file '{pattern}' did not match anything, skipping.", file=sys.stderr)
                continue
        for p in matches:
            if p not in seen:
                seen.add(p)
    ...
    if not expanded_files:
        print("No input files found after expanding patterns. Exiting.", file=sys.stderr)
        sys.exit(1)
    ...
    for input_path in expanded_files:
